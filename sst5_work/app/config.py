...
class Settings(BaseModel):
    ...
    def __init__(self, **data):
        ...
        self.APP_URL = os.getenv("APP_URL", "").strip()
        ...
        # APP_URL validation - canonical/sitemap URLs must never fall back to localhost in production
        env_app_url = os.getenv("APP_URL", "").strip().rstrip("/")
        if self.is_production:
            if not env_app_url:
                raise RuntimeError(
                    "CRITICAL CONFIGURATION ERROR: APP_URL environment variable is missing or empty in production mode. "
                    "Set APP_URL to the public HTTPS URL of this deployment."
                )
            if not env_app_url.startswith(("https://", "http://")):
                raise RuntimeError(
                    "CRITICAL CONFIGURATION ERROR: APP_URL must be an absolute HTTP(S) URL in production mode."
                )
            if "localhost" in env_app_url.lower() or "127.0.0.1" in env_app_url:
                raise RuntimeError(
                    "CRITICAL CONFIGURATION ERROR: APP_URL must not point to localhost or loopback in production mode."
                )
            self.APP_URL = env_app_url
        else:
            self.APP_URL = env_app_url

        # Google OAuth redirect URI validation - never use localhost in production.
        env_google_redirect = os.getenv("GOOGLE_REDIRECT_URI", "").strip()
        if self.is_production:
            if not env_google_redirect:
                raise RuntimeError(
                    "CRITICAL CONFIGURATION ERROR: GOOGLE_REDIRECT_URI environment variable is missing or empty in production mode."
                )
            if not env_google_redirect.startswith(("https://", "http://")) or "localhost" in env_google_redirect.lower() or "127.0.0.1" in env_google_redirect:
                raise RuntimeError(
                    "CRITICAL CONFIGURATION ERROR: GOOGLE_REDIRECT_URI must be a valid public HTTP(S) callback URL in production mode."
                )
            self.GOOGLE_REDIRECT_URI = env_google_redirect
        else:
            self.GOOGLE_REDIRECT_URI = env_google_redirect
